...
\textbf{Figure6. spaghetti plot for dental data}
...
\textbf{Figure7. boxplot for dental data mean and s.d.}
...
\textbf{Figure8. spaghetti plot for standardized dental data}
...
\textbf{Figure9. median polish and ACF for dental data}
...
\textbf{Figure10. SPLOM for dental data}
...
\textbf{Figure11. boxplot for plasma data mean and s.d.}
...
\textbf{Figure12. spaghetti plot for standardized plasma data}
...
\textbf{Figure13. median polish and ACF for plasma data}
...
\textbf{Figure14. SPLOM for plasma data}
...
(a) we assume non parallelism, so we test two groups seperately by using one-sample Hotelling's $T^2$ test:
...
x12 = plasma %>% 
  filter(group == 2) %>% 
  select(p0:p3)
C1 = rbind(diag(5), rep(-1,5))
H11 = HotellingsT2(as.matrix(x11) %*% C1)

```

* for control group, we calculate the pvalue = `r round(H11$p.value, 3)` < 0.05, so we reject the null hypothesis and conclude that there is not enough evidence to support that control group means are the same at all six measurement times

```{r}
H12 = HotellingsT2(as.matrix(x12) %*% C1)

```

* for obese group, we calculate the pvalue = `r round(H11$p.value, 3)` < 0.05, so we reject the null hypothesis and conclude that there is not enough evidence to support that obese group means are the same at all six measurement times
